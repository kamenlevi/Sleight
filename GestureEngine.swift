...
final class GestureEngine {
    ...
    private enum Tuning {
        ...
        //                 (= dialStartRotation, so the two recognizers are
        //                 exactly mutually exclusive)
        ...
        static let swipeMaxRotation: Float = 0.10
    ...
    private func updateTracking(_ touches: [Touch], at time: Double) {
        ...
        // Deliberately NOT gated on config.freezeScreen: for the dials the
        // freeze is a nicety, but the swipe is unusable without it — Mission
        // Control fires from the same fingers at less travel than the
        // deadzone, so it must be suppressed or it wins every time.
